...
describe("one question's answers", () => {
  it("asks the question's own address", async () => {
    const mock = ok({ current: null, earlier: [], question_notes: [] });
  ...
  it("accepts a null current — she has not answered yet", async () => {
    ok({ current: null, earlier: [], question_notes: [] });
    await expect(fetchQuestionAnswers("q-1")).resolves.toEqual({
      current: null,
      earlier: [],
      question_notes: [],
    });
  ...
  it("REFUSES a payload missing `question_notes` rather than showing no notes", async () => {
    // An absent list would render "no notes from Chuck" over notes the server
    // never sent (CC_TASK_REVIEW_LOOP_v1).
  ...
  it("REFUSES a payload missing `earlier` rather than rendering no history", async () => {
    ...
    ok({ current: null, question_notes: [] });
    await expect(fetchQuestionAnswers("q-1")).rejects.toThrow(/contract mismatch/);
  });
