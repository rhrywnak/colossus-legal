...
-->
# Discovery Response Entity Extraction — Pass 1: Entities Only (v5.4)

<!-- v5.4 CHANGE NOTE (stripped before reaching the LLM):
Two changes, no change to WHAT is extracted:
  1. THE DISCOVERY QUIRK, added (B2 §2, task P3). A sworn discovery ANSWER has two
     dates around it — the day it was verified, and any date the answer itself
     states — and only the second is `event_date`. The rule is spelled out with a
     worked case, because this is the template where the two are easiest to
     confuse and where confusing them would date a 2009 event to 2010.
  2. `statement_date` REMOVED from the shared ISO paragraph. The paragraph was
     copied across all pass-1 templates naming both properties, but
     discovery_response_schema_v5_1.yaml declares only `event_date`. The template
     was instructing the model to emit a property its schema does not have.
     Measured 2026-08-15: `statement_date` exists in exactly one schema
     (affidavit_schema_v5_1.yaml); complaint, court_ruling and this one all named
     it in prose and none of them declares it.
NOTE: the ISO paragraph itself, the closed pattern_tags vocabulary and the
canonical-name rule are UNCHANGED from v5.3 — they shipped there on 2026-07-31.
This bump is the quirk plus the property correction, nothing else.
-->
...
**The discovery quirk — when a sworn ANSWER is dated, and by what.**
A sworn discovery answer has two dates around it and they are not the same date:

- the date the ANSWER was given — the response or verification date, the day the responding party swore to it; and
- any date the answer TALKS ABOUT — the letter it mentions, the meeting it describes, the transfer it admits.

`event_date` records the second kind ONLY: the date of the thing the answer discusses. It is never the response date, and it is never the date of the interrogatory that was asked.

A concrete case. The interrogatory asks about a letter of 5 November 2009. The response is verified on 16 August 2010, and the answer reads *"Correspondence was received from Marie Awad in November of 2009."* Then `event_date` is `2009-11`, because that is the date the ANSWER states. `2010-08-16` — the day it was sworn — belongs to the response as a whole and is not this statement's `event_date`. It arrives separately, as the document's own date, entered by a human at intake.

And if an answer states no date at all — *"Not that I recall."* — omit `event_date`. Do not reach for the verification date to fill the gap. An undated admission is an honest undated admission; an admission stamped with the day it was sworn is a false claim about when the thing happened, and every chronology built on this graph would inherit it.
