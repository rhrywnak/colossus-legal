# Affidavit Entity Extraction — Pass 1: Entities Only (v5.3)

<!-- v5.3 CHANGE NOTE (stripped before reaching the LLM):
Three tightenings, no change to what is extracted:
  1. ISO-8601 date discipline for event_date AND statement_date (shared paragraph,
     all pass-1 templates). The v5.1 worked example emitted a RANGE
     ("2019-01 to 2020-06"); ranges are no longer permitted — the start date goes
     in the property, the span stays in the verbatim quote. The example is corrected.
  2. pattern_tags becomes a CLOSED vocabulary — "use ONLY these" — and the property
     is OMITTED when no tag applies (never an empty string). The v5.1 taxonomy was
     an undefined inline list; the tags are now enumerated and exclusive.
  3. Canonical-name rule made explicit and uniform across all pass-1 templates.
     This template already said "use canonical names"; it now says HOW to choose one.
NOTE: this is a PASS-1 bump only. Whether affidavit PASS-2 gains the v5.3 edge
deltas (CHARACTERIZES->Allegation, ABOUT->Allegation), and whether the affidavits
ride the reprocess, are open questions deferred to reprocess scoping.
-->
...
- **Parties** must use the same canonical names as in the complaint and other documents. If the complaint names "George Phillips" and the affidavit refers to "Attorney Phillips," your Party entity must use "George Phillips" so the system can connect them across documents. The canonical-name rule below says exactly how to choose that one name.
...
### 1. Caption / Title
- **Contains:** Court name, case name, title (e.g., "AFFIDAVIT OF [NAME]")
...
- **Extract from here:** Party entity for the affiant with role="affiant". If the preamble contains case-relevant facts (e.g., "I served as the daily caregiver for [person] from 2008 to 2009"), also extract as an Evidence entity — this establishes the affiant's basis for testimony.
...
- **How to recognize substantive statements:** "I observed...", "I was present when...", "[Person] stated to me that...", "In my professional opinion...", "I have personal knowledge that..."
...
### IMPORTANT: Multiple Affidavits in One PDF

Some PDFs contain multiple separate affidavits by the same person, each with its own signature block and notarization. Treat each as a separate group of sworn statements — extract Evidence entities from ALL affidavits in the document. Use paragraph numbering that distinguishes them (e.g., if two affidavits both start at paragraph 1, use "A-1", "A-2" for the first and "B-1", "B-2" for the second, or simply restart numbering and let the page_number disambiguate).
...
- `role`: affiant, witness, plaintiff, defendant, attorney, caregiver, decedent, third_party, interested_party, guardian_ad_litem, personal_representative
- `party_type`: "person" or "organization"
- `aliases`: Other names or references used for this party, comma-separated (e.g., "Attorney Phillips, Mr. Phillips")
...
**Properties:**
- `title`: Short descriptive title summarizing what the statement establishes (e.g., "Jones: Robert Smith was alert and capable of making own decisions")
- `answer`: The substance of the sworn statement in your own summary
- `paragraph`: Paragraph number in the affidavit (e.g., "5", "12") — REQUIRED for every Evidence entity
...
- `statement_type`:
  ...
  - "factual_assertion" — stating a specific observed fact (e.g., "I saw Mr. Smith sign the document")
  - "expert_opinion" — professional judgment by a qualified witness
- `significance`: REQUIRED — why this statement matters for the case. Reference specific complaint allegations or legal theories when applicable. Use prefixes:
...
- `weight`: 1-10 importance (10 = most probative for trial). Direct firsthand observations typically 7-9. Conclusions or beliefs 4-6.
- `statement_date`: Date the affidavit was signed/notarized (same for all Evidence entities from one affidavit) — ISO-8601, see below
- `event_date`: Date of the event being described, if the statement mentions when something happened — ISO-8601, see below
- `pattern_tags`: Comma-separated tags from the CLOSED vocabulary — see below; omit entirely if none apply
...
**CLOSED VOCABULARY — use ONLY these tags.** These are the trial-prep query patterns a sworn affidavit can evidence.
...
Read through the entire affidavit. Extract every person and organization mentioned by name. Use canonical names matching other case documents where possible. Include aliases when the document uses multiple names for the same person.
...
- Set page_number and paragraph number (both REQUIRED)
- Assess the significance — does this corroborate a complaint allegation? Rebut a defendant's claim? This field is REQUIRED.
- Rate the weight based on how important this testimony is for the case
- Set statement_date from the notarization date
...
- Statements about the decedent's competence or capacity → pattern_tags: "competence"
- Statements about pressure, threats, or manipulation → pattern_tags: "coercion" or "undue_influence"
- Statements about mistreatment of an elderly or vulnerable person → pattern_tags: "elder_abuse"
...
> Signed and notarized March 15, 2020.
  ...
  "entities": [
    ...
    },
    {
      ...
      "properties": {
        ...
        "significance": "Establishes Jones's basis for testifying about Smith's condition and wishes",
        "weight": 7,
    ...
    },
    {
      ...
      "properties": {
        ...
        "evidence_strength": "sworn_testimony",
        "statement_type": "factual_assertion",
        "significance": "CORROBORATES: Smith opposed having a conservator appointed over his objections",
        "weight": 9,
        "statement_date": "2020-03-15",
...
- statement_date set from the notarization date on ALL Evidence entities
- pattern_tags used where applicable (competence)

**What was NOT extracted:**
- Paragraph 1 was NOT Evidence — formulaic identity statement
- "Affiant" was not extracted as a Party — it's a role, not a name
- "FURTHER AFFIANT SAYETH NOT" was not extracted — formulaic attestation
...
- [ ] Did I use canonical names consistent with other case documents (check the context block)?
...
- [ ] Does every Evidence have a significance value (REQUIRED)?
- [ ] Did I set statement_date from the notarization date?
- [ ] If the PDF contains multiple affidavits, did I extract from ALL of them?

**Negative checks:**
- [ ] Did I avoid extracting purely formulaic paragraphs ("I am over 18 years of age") as Evidence?
- [ ] Did I avoid extracting the attestation/conclusion ("Further affiant sayeth not") as Evidence?
...
- [ ] Did I avoid combining multiple numbered paragraphs into a single Evidence entity?
