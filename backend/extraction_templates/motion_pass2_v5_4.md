...
- TWO departures that invert the transcript:
  1. STATED_BY was UNIFORM (every Evidence -> the movant), like court_ruling's judge. As of v5.4 it is not created at all — see the v5.4 note below; the uniformity is exactly why.
...
Also in v5.4 — BAR A: this type creates NO STATED_BY edges. Its speaker never
varies, and the census measured what that costs: an appellate brief published 62
STATED_BY edges all pointing at one Party, 14% of its edge count. The speaker
survives as the `movant` property on every Evidence entity, which is why the bar
applies to this type and NOT to court_ruling, discovery_response or affidavit,
where the census found no equivalent node property to fall back on.

Also in v5.4 — R-4: `legal_standard` entities emit ZERO edges of any kind. The
Stage 1b table always said so while Stage 0 said structural edges are created
"for ALL Evidence"; Stage 0 now names the exception.

-->
# Motion Relationship Extraction — Pass 2: Relationships Only (v5.4)
...
- **"Who claimed this?"** → the `movant` property carried on every Evidence entity. **Not an edge** — see §1.
...
**A motion has one voice, and that is exactly why the edge is worthless here.** The filing speaks for the party on whose behalf it was filed, so every STATED_BY edge in the document would point at the same Party. An edge whose target is knowable before the document is opened carries no information. Measured on the sibling appellate-brief type under the previous template: 62 STATED_BY edges, 62 of them to the same Party, 14% of that document's entire edge count.

**The movant is not lost — it was never on the edge.** It is the `movant` property, carried identically on every Evidence entity Pass 1 produced and written to the graph node. "Who filed this assertion?" is answered by reading that property. An edge is for a fact that varies between assertions; a property is for one that does not.

**⚠ This is the OPPOSITE of the rule for a hearing transcript, and the reason is the same fact seen from the other side.** A transcript's speaker CHANGES with every turn, so there the edge carries real information and the transcript template keeps it. Here the speaker never changes.

**⚠ Do not substitute something else for the missing edge.** In particular, do NOT create ABOUT → the movant on every assertion to compensate. That rebuilds the same constant edge under another name. ABOUT is created only where §2's test is independently met.
...
- `"relationship_type"`: "ABOUT", "CONTRADICTS", "REBUTS", or "CHARACTERIZES" — **"STATED_BY" is not created by this document type at all (§1)**; an edge of that type in your output is an error, not a bonus
- `"from_entity"`: the entity ID of the source (always an Evidence entity for motions)
- `"to_entity"`: the entity ID of the target — a Party **or** a `ctx:allegation-NNN` Allegation for ABOUT and CHARACTERIZES; a `ctx:allegation-NNN` complaint entity ID for CONTRADICTS/REBUTS
...
**Note that there is no STATED_BY edge anywhere in this example** — not for evidence-007, not for evidence-042, not for any entity; the movant is the `movant` property (§1). evidence-042 is a recitation: it keeps its structural ABOUT and produces no finding-edge.
...
**THE BAR CHECKS — each has a required answer of ZERO:**
...
- [ ] Did I resist creating ABOUT → the movant on every assertion to replace the STATED_BY edge this type no longer has? (§1's second hazard)
