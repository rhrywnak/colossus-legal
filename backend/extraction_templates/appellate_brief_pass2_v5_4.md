...
v5.4 (2026-08-25) — THE STRUCTURAL-EDGE BAR. Three changes, all from the
measured census of doc-penzien-coa-brief-03-14-2011 (457 published edges from
62 assertions — 7.19 edges per assertion against the discovery-response peer's
3.25). v5.3 is preserved unedited as the record of what produced those edges.
  1. BAR A — STATED_BY is GONE from this type (was §1). All 62 of the brief's
     STATED_BY edges pointed at the same Party. An edge whose target is knowable
     before reading the document carries no information, and this one cost 14% of
     the document's edges. The filer survives as `filed_by` on every Evidence
     node, which the census confirmed present on 62 of 62 — this type is one of
     only three where dropping the edge loses nothing.
  2. BAR B — CHARACTERIZES REPLACES ABOUT on the same (source, target) pair.
     v5.3's §6 said the two were "not either/or"; the result was that all 66
     CHARACTERIZES edges sat on a pair that already carried an ABOUT — 100%
     duplication. The stronger edge now wins the pair outright.
  3. R-4 — `legal_standard` entities emit ZERO edges. v5.3's Stage 1b table
     already said so while Stage 0 said structural edges are created "for ALL
     Evidence"; the output followed the looser half (12 edges). Stage 0 now
     names the exception.
Deliberately NOT changed: §3 CORROBORATES-never, §5 REBUTS (both target
classes), the Stage 1 recitation gate — the census measured that gate holding
at 100% and it stays exactly as written.
-->
# Appellate Brief Relationship Extraction — Pass 2: Relationships Only (v5.4)
...
- **"Who claimed this?"** → the `filed_by` property carried on every Evidence entity. **Not an edge** — see §1.
...
### 1. STATED_BY — NONE. THIS DOCUMENT TYPE CREATES NO STATED_BY EDGES.

**Rule:** Create ZERO STATED_BY relationships. Not one, from any Evidence entity, including recitations.

**A brief has one voice, and that is exactly why the edge is worthless here.** The filing speaks for the party on whose behalf it was filed, so every STATED_BY edge in the document would point at the same Party. The previous version of this template required one per assertion, and the last brief processed under it produced **62 STATED_BY edges, 62 of which pointed at Marie Awad** — 14% of that document's entire edge count, spent restating a single fact that was already known before the document was opened.

**The filer is not lost — it was never on the edge.** It is the `filed_by` property, carried identically on every Evidence entity Pass 1 produced, and it is written to the graph node. "Who filed this assertion?" is answered by reading that property. An edge is for a fact that varies between assertions; a property is for one that does not.

**⚠ This is the OPPOSITE of the rule for a hearing transcript, and the reason is the same fact seen from the other side.** A transcript's speaker CHANGES with every turn, so there the edge carries real information and the transcript template keeps it. Here the speaker never changes. Do not carry a habit from that type into this one.

**⚠ Do not substitute something else for the missing edge.** In particular, do NOT create ABOUT → the filing party on every assertion to compensate. That would rebuild the same constant edge under a different name. ABOUT is created only where §2's test is independently met — where the assertion genuinely concerns that party.
...
**⚠ HARD GUARD — ONE PAIR, ONE EDGE: ABOUT YIELDS TO CHARACTERIZES.** If you are going to create CHARACTERIZES from an assertion to a target (§6), do **not** also create ABOUT from that same assertion to that same target. CHARACTERIZES already establishes that the assertion concerns that target, and says something stronger besides. Two edges on one pair record one fact twice.

The rule is mechanical, so apply it mechanically: **before writing any ABOUT, ask whether this same (assertion → target) pair is getting a CHARACTERIZES. If yes, the ABOUT is not written.** This applies to Party and Allegation targets alike.

*(Measured, so you know the size of what this prevents: under the previous version every one of the last brief's 66 CHARACTERIZES edges sat on a pair that already carried an ABOUT. Not most — all 66.)*
...
→ **Create NO edge from `evidence-014`.** It is the appellant's own claim, reproduced here to be denied; it belongs to the appellant's brief, which is processed as its own document and carries its own `filed_by`. Creating a finding-edge from it would record the appellee as having asserted the appellant's claim.
→ Both nodes still get their ABOUT edges.
...
A single assertion may carry CHARACTERIZES to **both** a Party and an Allegation — they answer different questions ("who was labelled?" and "which claim does the labelling touch?"). Those two are not either/or, because they are two different targets.

**⚠ But CHARACTERIZES and ABOUT to the SAME target ARE either/or, and CHARACTERIZES wins.** Where you create CHARACTERIZES from an assertion to a target, that pair gets no ABOUT edge (§2's hard guard). CHARACTERIZES is strictly stronger: it establishes the topical connection ABOUT would have carried, and adds the evaluative claim on top. Writing both records one connection twice and inflates every count that reads this document.

**This reverses the previous version of this template**, which told you the two were additive on the same pair. Following that instruction, the last brief processed produced 66 CHARACTERIZES edges of which 66 duplicated an ABOUT. If you have that pattern in mind from an earlier template, drop it.
...
- **If `attribution` is `recitation` → this node produces NO finding-edge of any kind.** Do NOT create CONTRADICTS, REBUTS, or CHARACTERIZES from it. (CORROBORATES is unavailable to every node in this document — see §3.) The filer was reproducing someone else's position, not asserting it. The quoted material belongs to the document it came from — the opposing brief, the opinion under review, the transcript — which is processed separately and will carry its own edges there; creating a finding-edge here would extract that document's content a second time, attributed to the wrong party.
  - A recitation STILL gets its ABOUT edges — those are structural, not finding-edges. ABOUT is not gated for either target. (It gets no STATED_BY, but neither does anything else in this document type — §1.)
...
**Then reconcile against Stage 0.** Every pair that just received a CHARACTERIZES must NOT also appear as an ABOUT. If Stage 0 wrote an ABOUT on a pair this stage has now characterized, **remove that ABOUT** — the pair keeps CHARACTERIZES alone. Do this before you run the checklist; it is the single most common way the old duplication comes back.

### STAGE 0 — Structural edges (recitations included; `legal_standard` excluded)

There is now exactly ONE structural edge type, and it has one exception.

1. **STATED_BY: none.** This type creates no STATED_BY edges at all (§1). There is no count to reconcile against the entity list.
2. **ABOUT:** one per party each Evidence concerns — remembering the judge under review (§2) — AND one per Allegation whose subject matter it discusses. Created for recitations too. **Not created on any pair that will carry a CHARACTERIZES** (§2's hard guard).

**⚠ THE `legal_standard` EXCEPTION — structural does NOT mean unconditional.** An Evidence entity whose `statement_type` is `legal_standard` produces **no edges of any kind**, and that includes the structural ones in this stage. Stage 1b's table has always said so; the previous version of this stage said structural edges were created "for ALL Evidence", and the two could not both be true. The table wins. A recited rule of law is captured in the `legal_basis` property; it is not an assertion about anyone in this case, so there is nothing for it to be ABOUT.
...
- `"relationship_type"`: "ABOUT", "CONTRADICTS", "REBUTS", or "CHARACTERIZES"
- `"from_entity"`: the entity ID of the source (always an Evidence entity for briefs)
- `"to_entity"`: the entity ID of the target — a Party **or** a `ctx:allegation-NNN` Allegation for ABOUT and CHARACTERIZES; a `ctx:allegation-NNN` Allegation **or** a `ctx:`-prefixed Evidence node from another brief for CONTRADICTS/REBUTS

Note that neither "CORROBORATES" nor "STATED_BY" appears in that list. CORROBORATES is unavailable to this document type (§3); STATED_BY is not created by this document type at all (§1). An edge of either type in your output is an error, not a bonus.
...
Note four things. **There is no STATED_BY edge anywhere in this example** — not for evidence-014, not for evidence-015, not for any entity; the filer is the `filed_by` property (§1). evidence-014 is a recitation: it gets its structural ABOUT → Allegation but no finding-edge. evidence-015, the denial that accompanies that recitation, carries the brief-to-brief REBUTS to the opposing brief's assertion. And **evidence-031 appears twice with CHARACTERIZES and never with ABOUT** — those two pairs are characterized, so §2's hard guard withholds the ABOUT that an earlier version of this template would have added.
...
- [ ] Did I still create ABOUT for those recitation nodes (the one remaining structural edge type is not gated)?
- [ ] Did I still create ABOUT → Allegation for recitation nodes whose subject matter an Allegation concerns?

**THE THREE BAR CHECKS — count these first, because each has a required answer of ZERO:**
- [ ] **STATED_BY: is the count ZERO?** Not "one per entity", not "all the same party" — zero, across the whole output. (§1)
- [ ] **CORROBORATES: is the count ZERO?** From every statement type, including heavily record-cited factual assertions and counsel's descriptions of attached affidavits. (§3)
- [ ] **Edges from `legal_standard` entities: is the count ZERO?** Every type, structural ones included. (Stage 0, Stage 1b)

**The pair check — the one that is easy to fail silently:**
- [ ] For every CHARACTERIZES I created, did I confirm there is **no ABOUT** from the same Evidence entity to the same target? Walk the CHARACTERIZES list and check each one; do not assume.
- [ ] If Stage 0 wrote an ABOUT that Stage 2b later characterized, did I **remove** that ABOUT?
...
- [ ] Did I resist creating ABOUT → the filing party on every assertion to replace the STATED_BY edge this type no longer has? (§1's second hazard)
