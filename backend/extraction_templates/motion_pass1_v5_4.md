...
-->
# Motion Entity Extraction — Pass 1: Entities Only (v5.4)

> **v5.4 — THE MATERIALITY BAR.** Measured on 2026-08-27: a 17-page motion
> (~5,900 words) extracted under v5.3 produced output that filled a
> 64,000-token cap — one entity per ~21 words, each restating its own quote
> across four fields. The mechanism, not the model, was the cost. v5.4 adds
> the materiality bar (one entity per unit of work, merged spans, field
> caps, and a budget sanity check). No classification rule changed.
...
## ⚠ THE MATERIALITY BAR — READ BEFORE EXTRACTING ANY EVIDENCE

**One entity per UNIT OF WORK, not per sentence.** A unit of work is a
passage doing ONE thing: making one factual claim, one characterization,
one relief demand, one applied-law argument, or reciting one opposing
position. Adjacent sentences that continue the SAME work — same
`statement_type`, same `attribution`, same `asserted_against`, one
narrative point — are ONE entity whose `verbatim_quote` spans them.
Sentences doing DIFFERENT work split, exactly as Examples 1–2 show.

**Do not extract at all:**
- Transition or summary sentences that restate an assertion this output
  already carries ("As shown above…", "In sum…" when it adds nothing new).
- Procedural apparatus: hearing notices, "relies on the attached brief,"
  concurrence-request recitals, the caption and signature machinery.
- Bare legal boilerplate (already excluded — the rule stands).

**Field caps — an entity states its point ONCE:**
- `title`: at most 10 words.
- `summary`: ONE sentence, at most 25 words. It may not repeat the title.
- `significance`: ONE sentence, at most 25 words, and only what neither
  title nor summary already says.
- `verbatim_quote`: the operative filed text, contiguous, at most ~80
  words. For a longer passage quote its single most operative sentence in
  full — never an ellipsis, never a stitched quote: the quote must appear
  in the document exactly as written.

**The budget sanity check.** A motion yields roughly one Evidence entity
per 60–120 words of in-scope text. A 6,000-word motion is ~50–100
entities. If your count runs far above that band, you are splitting one
unit of work into fragments — go back and merge before returning. This is
a sanity band, not a quota: never pad toward it, never discard a distinct
unit of work to stay inside it.
...
### Evidence
Each UNIT OF WORK the motion performs is ONE Evidence entity (see THE MATERIALITY BAR above). This is the core extraction target.
...
**Note this is a SEPARATE entity from Example 1** — not because the sentences are adjacent, but because they do DIFFERENT work: one states a fact, the other characterizes. Different statement_types, different edges. **Had the second sentence continued the factual narrative instead ("The Defendant also withheld the bank statements"), the two would MERGE into one entity** with a spanning quote — see the bar, and Example 8.
...
### Example 8 — NEGATIVE: one narrative, three sentences → ONE entity, not three

From a Statement of Facts (page 4): "On August 3, 2010, CFS filed a final account of fiduciary. The account showed a balance on hand of $50,420. The attached schedule of fees and claims totaled $68,637."

Three sentences, one unit of work — a single factual narrative about one filing, one statement_type, one attribution, one target. ONE entity:
- title: "CFS final account: $50,420 on hand, $68,637 claimed"
- statement_type: "factual_assertion" · attribution: "own_determination"
- **verbatim_quote:** all three sentences, contiguous.

Splitting this into three entities records one fact three times and triples the output cost of every similar passage. That is the pattern the materiality bar exists to stop.
...
Work through the motion in order. For each UNIT OF WORK (the materiality bar governs what counts as one):
...
- [ ] Did I create one entity per UNIT OF WORK — each lettered ground, each distinct claim — and MERGE adjacent sentences continuing the same work?
- [ ] Is my entity count inside the budget band (one per ~60–120 words of in-scope text)? If far above, did I go back and merge?
- [ ] Are title (≤10 words), summary (≤25), and significance (≤25, non-repeating) within their caps on every entity?
