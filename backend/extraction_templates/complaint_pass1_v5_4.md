...
-->
# Civil Complaint Entity Extraction — Pass 1: Entities Only (v5.4)

<!-- v5.4 CHANGE NOTE (stripped before reaching the LLM):
One change, no change to WHAT is extracted: `statement_date` REMOVED from the
shared ISO paragraph. The paragraph was copied across all pass-1 templates naming
both `event_date` and `statement_date`, but complaint_schema_v5_1.yaml declares
only `event_date` — so the template was instructing the model to emit a property
its schema does not have. Measured 2026-08-15: `statement_date` exists in exactly
one schema (affidavit_schema_v5_1.yaml).
NOTE: the ISO paragraph itself and the canonical-name rule are UNCHANGED from
v5.3 — they shipped there on 2026-07-31.
DOMAIN NOTE: the Complaint is the case anchor and is do-not-reprocess (ruled
2026-08-13). This file exists so the template set is correct and consistent; it
does not imply the Complaint will be re-run under it.
-->

<!-- v5.3 CHANGE NOTE (stripped before reaching the LLM):
Two tightenings, no change to what is extracted:
  1. ISO-8601 date discipline for event_date (shared paragraph, all pass-1s).
  2. Canonical-name rule made explicit and uniform, and the `aliases` property —
     already declared in complaint_schema_v5_1.yaml but missing from this
     template's Party property list — is now documented so it is actually emitted.
This template extracts Allegation/LegalCount/Harm, not Evidence, so it carries no
pattern_tags taxonomy; the closed-vocabulary ruling does not apply here.
-->
...
- `event_date`: Date of the alleged conduct if mentioned — ISO-8601, see below
