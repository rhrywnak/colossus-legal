...
v5.4 (2026-08-25) — BAR B, the duplicate-pair bar. From the measured census of
doc-penzien-coa-brief-03-14-2011: all 66 of that document's CHARACTERIZES edges
sat on a (source, target) pair that already carried an ABOUT, and the
discovery-response peer's rate was 11 of 16. §2 gains a hard guard and §6's
"not either/or" is reversed for the same-target case: CHARACTERIZES replaces
ABOUT on a pair, rather than joining it. Nothing else in this template changed.
The previous version is preserved unedited as the record of what produced the
existing edges.

-->
# Discovery Response Relationship Extraction — Pass 2: Relationships Only (v5.4)

<!-- v5.3 CHANGE NOTE (stripped before reaching the LLM):
ADDITIVE coverage only — two new legal targets for edge types that already existed:
  1. CHARACTERIZES -> Allegation (§6): a characterization can bear on what an
     Allegation asserts about a party, not just on the party. Party-target
     CHARACTERIZES is unchanged.
  2. ABOUT -> Allegation (§2): topical reach, so a NEUTRAL statement that discusses
     an Allegation's subject is reachable from that Allegation without any claim
     about direction. Carries a hard guard: ABOUT is topical, never directional.
NOTHING in the v5.2 semantics changed. The corroboration bar (MCR 2.309/2.313,
Radtke, Step A/B/C), denial routing, and REBUTS->Allegation are untouched. The
output JSON format gains only new to_entity targets for two existing edge types —
no new relationship_type, no parser/ingest impact.
-->
...
**Second target — ABOUT → Allegation (topical reach).** Create ABOUT → Allegation when this Q&A **discusses the subject matter** an Allegation concerns. ABOUT answers "what is this statement *about*?" — nothing more.

**⚠ HARD GUARD — ABOUT IS TOPICAL, NEVER DIRECTIONAL.** ABOUT carries **no** support, opposition, confirmation, or denial. It does **not** mean the answer helps the Allegation, hurts it, corroborates it, or rebuts it. Those are CORROBORATES (§3) and REBUTS (§5), decided by their own tests and only from the answer's words.

The whole point of ABOUT → Allegation is reach for a **neutral** answer: one that touches an Allegation's subject while conceding nothing and countering nothing. Such an answer gets ABOUT and no polarity edge — and that is the correct, complete result, not an omission to fix.

If you find yourself reasoning *"this supports/undercuts the allegation, so ABOUT"* — stop. That reasoning belongs to CORROBORATES/REBUTS. Ask instead: *"is this Allegation's subject matter what the answer is discussing?"*

ABOUT → Allegation is **additive** to ABOUT → Party. A Q&A is normally ABOUT both the parties it discusses and the Allegations whose subject it touches.

**⚠ HARD GUARD — ONE PAIR, ONE EDGE: ABOUT YIELDS TO CHARACTERIZES.** If you are going to create CHARACTERIZES from a node to a target (§6), do **not** also create ABOUT from that same node to that same target. CHARACTERIZES already establishes that the node concerns that target, and says something stronger besides. Two edges on one pair record one fact twice.

Apply it mechanically: **before writing any ABOUT, ask whether this same (source → target) pair is getting a CHARACTERIZES. If yes, the ABOUT is not written.** Party and Allegation targets alike.

*(Measured, so you know the size of what this prevents: under the previous templates, every one of an appellate brief's 66 CHARACTERIZES edges sat on a pair that already carried an ABOUT — all 66 — and a discovery response's rate was 11 of 16.)*

**Worked example — ABOUT → Allegation (neutral answer, no polarity edge).**
Evidence (Q73), statement_type=referral: answer = "The disposition of the $50,000 is reflected in the accountings filed with the probate court."
Allegations in context: ctx:allegation-012 ("CFS took possession of the $50,000 check"), ctx:allegation-019 ("The $50,000 was never accounted for to the estate").
→ The answer discusses the $50,000 — the subject of both Allegations. It concedes nothing (a `referral`; §3 bars CORROBORATES) and denies nothing (§5 bars REBUTS). **Create ABOUT → ctx:allegation-012 and ABOUT → ctx:allegation-019, and NO polarity edge.** The answer is now reachable from both Allegations without any claim about which way it cuts.

This ABOUT → Allegation target requires cross-document context. If the context block contains no Allegation entities, create ABOUT to Parties only.
...
Judge it from the Allegation's text. If the Allegation asserts the party behaved unreasonably and the answer labels that same behaviour unreasonable, the characterization bears on it. If the answer labels the party in a way the Allegation says nothing about, target the Party only.

A single answer may carry CHARACTERIZES to **both** a Party and an Allegation — they answer different questions ("who was labelled?" and "which claim does the labelling touch?"). Those two are not either/or, because they are two different targets.

**⚠ But CHARACTERIZES and ABOUT to the SAME target ARE either/or, and CHARACTERIZES wins.** Where you create CHARACTERIZES from a node to a target, that pair gets no ABOUT edge (§2's hard guard). CHARACTERIZES is strictly stronger: it establishes the topical connection ABOUT would have carried and adds the evaluative claim on top. Writing both records one connection twice and inflates every count that reads this document.

**This reverses the previous version of this template**, which told you the two were additive on the same pair.

**This is not a polarity edge.** CHARACTERIZES → Allegation says the answer *labels the party in terms the Allegation is about*. It does not say the answer confirms or defeats the Allegation — that is CORROBORATES/REBUTS, decided separately. An answer may legitimately carry CHARACTERIZES → Allegation *and* REBUTS → that same Allegation.

**Worked example — CHARACTERIZES → Party AND → Allegation.**
Evidence (Q77), statement_type=partial_admission: answer = "I may have used that characterization — I found her filings largely unintelligible."
Allegation (ctx:allegation-044): "Phillips repeatedly disparaged Marie Awad's filings to the probate court as incoherent."
→ The answer labels Marie's filings "unintelligible" — the exact evaluative conduct the Allegation asserts. **Create CHARACTERIZES → party (Marie Awad) AND CHARACTERIZES → ctx:allegation-044.**

The Allegation target requires cross-document context. If the context block contains no Allegation entities, create CHARACTERIZES to Parties only.
...
For each Evidence entity, read the question and answer. Identify every party the Q&A discusses. Create one ABOUT relationship for each. Then check each Allegation in the context block: does this Q&A discuss its subject matter? Create ABOUT → Allegation for each, on topic alone (§2 guard — never because the answer seemed to support or oppose it).

### Step 3: Create all CHARACTERIZES relationships
Re-read each Evidence entity. If the answer contains evaluative language about a party, create a CHARACTERIZES relationship to that Party. Then check whether that characterization also bears on what an Allegation asserts about the party (§6) — if it does, create CHARACTERIZES → that Allegation as well.
...
- `"to_entity"`: the entity ID of the target — a Party entity for STATED_BY; a Party **or** a complaint Allegation for ABOUT and CHARACTERIZES; a complaint entity ID for CORROBORATES/CONTRADICTS/REBUTS
...
```json
{
  "relationships": [
    ...
    {"relationship_type": "ABOUT", "from_entity": "evidence-073", "to_entity": "allegation-012"},
    {"relationship_type": "CHARACTERIZES", "from_entity": "evidence-077", "to_entity": "allegation-044"}
  ]
}
```
...
**THE PAIR CHECK (v5.4) — the one that is easy to fail silently:**
- [ ] For every CHARACTERIZES I created, did I confirm there is **no ABOUT** from the same node to the same target? Walk the CHARACTERIZES list and check each one; do not assume. (§2's hard guard, §6)
...
- [ ] Did I create ABOUT → Allegation for Q&A pairs that discuss an Allegation's subject matter?
- [ ] Did I create ABOUT purely on TOPIC — never because an answer seemed to support or oppose an Allegation?
- [ ] For a neutral answer, did I leave it with ABOUT and NO polarity edge (the correct result, not a gap)?
