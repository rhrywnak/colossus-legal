...
- Departures from the appellate_brief chassis:
  1. filed_by -> author. STATED_BY targeted the author; as of v5.4 it is not created at all — see the v5.4 note below.
...
survives as the `author` property on every Evidence entity, which is why the bar
...
-->
# Correspondence Relationship Extraction — Pass 2: Relationships Only (v5.4)
...
- **"Who wrote this?"** → the `author` property carried on every Evidence entity. **Not an edge** — see §1.
...
**A letter has one voice, and that is exactly why the edge is worthless here.** The letter speaks for the person on whose behalf it was written, so every STATED_BY edge in the document would point at the same Party. An edge whose target is knowable before the document is opened carries no information. Measured on the sibling appellate-brief type under the previous template: 62 STATED_BY edges, 62 of them to the same Party, 14% of that document's entire edge count.

**The author is not lost — it was never on the edge.** It is the `author` property, carried identically on every Evidence entity Pass 1 produced and written to the graph node. "Who wrote this?" is answered by reading that property. An edge is for a fact that varies between assertions; a property is for one that does not.

**⚠ This is the OPPOSITE of the rule for a hearing transcript, and the reason is the same fact seen from the other side.** A transcript's speaker CHANGES with every turn, so there the edge carries real information and the transcript template keeps it. Here the author never changes.

**⚠ Do not substitute something else for the missing edge.** In particular, do NOT create ABOUT → the author on every assertion to compensate. That rebuilds the same constant edge under another name. ABOUT is created only where §2's test is independently met.
...
→ Those are the third party's words, reproduced. **Create NO CHARACTERIZES edge.** The words belong to their speaker. The node keeps its ABOUT edges.

**Second legal target — CHARACTERIZES → Allegation.** Create CHARACTERIZES → Allegation when the evaluative statement bears on **what an Allegation asserts about the party** — judged from the Allegation's text. A single assertion may carry CHARACTERIZES to **both** a Party and an Allegation — two different targets, so not either/or. **But CHARACTERIZES and ABOUT to the SAME target ARE either/or, and CHARACTERIZES wins:** that pair gets no ABOUT edge (§2's hard guard), because CHARACTERIZES already carries the topical connection and adds the evaluative claim on top. **This reverses the previous version of this template.** This is not a polarity edge — it says the assertion *labels the party in terms the Allegation is about*, not that it confirms or defeats it. A single assertion may carry CHARACTERIZES → an Allegation *and* REBUTS → that same Allegation.
...
- **If `attribution` is `recitation` → this node produces NO finding-edge of any kind.** Do NOT create CONTRADICTS, REBUTS, or CHARACTERIZES from it. (CORROBORATES is unavailable to every node in this document — see §3.) The author was reproducing a third party's words, not asserting them. Those words belong to their speaker; a finding-edge here would attribute them to the letter-writer.
  - A recitation STILL gets its ABOUT edges — structural, not finding-edges. (It gets no STATED_BY, but neither does anything else in this document type — §1.)
...
### STAGE 0 — Structural edges (recitations included)

There is now exactly ONE structural edge type.

1. **STATED_BY: none.** This type creates no STATED_BY edges at all (§1). There is no count to reconcile against the entity list. **Not created on any pair that will carry a CHARACTERIZES** applies to what remains — see item 2 and §2's hard guard.
...
- `"to_entity"`: the entity ID of the target — a Party **or** a `ctx:allegation-NNN` Allegation for ABOUT and CHARACTERIZES; a `ctx:allegation-NNN` Allegation **or** a `ctx:`-prefixed Evidence node from a filing/letter for CONTRADICTS/REBUTS
...
Note three things. **There is no STATED_BY edge anywhere in this example** — not for evidence-006, not for evidence-041, not for any entity; the author is the `author` property (§1). evidence-006 is an information_request: it gets ABOUT (to a party and to the Allegation it asks after) and nothing else. evidence-018's REBUTS targets a `ctx:evidence` node from a later filing — the letter contesting a claim made after it.
...
- [ ] Did I resist creating ABOUT → the author on every assertion to replace the STATED_BY edge this type no longer has? (§1's second hazard)
