...
- Mirrors discovery_response_pass2_v5_3.md. The court_ruling-specific addition is STAGE 1 (the recitation gate): a party_recitation Evidence node produces NO edge of any kind.
...
-->
# Court Ruling Relationship Extraction — Pass 2: Relationships Only (v5.4)
...
mirroring discovery_response_pass2_v5_3:
  1. CHARACTERIZES -> Allegation (§6) — GATED by Stage 1 like every finding-edge.
     A recited characterization is the PARTY's, not the court's, and produces no
     CHARACTERIZES edge to a Party OR to an Allegation.
  2. ABOUT -> Allegation (§2) — NOT gated. ABOUT is structural (Stage 0), so a
     party_recitation still gets its ABOUT edges, Party and Allegation alike: a
     recited position is still ABOUT its subject. Carries the hard topical guard.
The recitation gate itself, the polarity rules (Stage 2), CORROBORATES/REBUTS
semantics, and the finding-type restrictions are UNCHANGED. No new
relationship_type, no parser/ingest impact.
-->
...
**ABOUT IS STRUCTURAL — IT IS NOT GATED BY STAGE 1.** A `party_recitation` still gets its ABOUT edges, to Parties and to Allegations alike. The court restating a party's position is still *discussing* that subject; what the gate withholds is any claim that the court FOUND something (§Stage 1).
...
Allegation (ctx:allegation-031): "CFS charged the estate excessive and unjustified fees."
→ The recitation discusses the fees — the subject of the Allegation. **Create ABOUT → ctx:allegation-031** (structural, permitted for recitations) **and NO CORROBORATES/REBUTS/CHARACTERIZES** (Stage 1 gates every finding-edge). The party's position is now reachable from the Allegation it concerns, correctly marked as something the court recited rather than found.
...
**Second legal target — CHARACTERIZES → Allegation.** A characterization can bear on an Allegation as well as on a Party. Create CHARACTERIZES → Allegation when the court's own evaluative statement bears on **what an Allegation asserts about the party** — not merely on the party in general.

**The test:** *"Does this evaluative statement bear on what an Allegation asserts about the party?"*

Judge it from the Allegation's text. If the Allegation asserts the party behaved unreasonably and the finding labels that same behaviour unreasonable, the characterization bears on it. If the finding labels the party in a way the Allegation says nothing about, target the Party only.

A single finding may carry CHARACTERIZES to **both** a Party and an Allegation — they answer different questions ("who was labelled?" and "which claim does the labelling touch?"). Those two are not either/or, because they are two different targets.
...
**This is not a polarity edge.** CHARACTERIZES → Allegation says the finding *labels the party in terms the Allegation is about*. It does not say the finding confirms or defeats the Allegation — that is CORROBORATES/REBUTS, decided separately. A finding may legitimately carry CHARACTERIZES → Allegation *and* REBUTS → that same Allegation.

**CRITICAL — the Stage-1 gate applies to BOTH targets.** CHARACTERIZES → Allegation is a finding-edge exactly as CHARACTERIZES → Party is. A `party_recitation` produces NEITHER: the party is characterizing, not the court.

**Worked example — the court's OWN characterization → Party AND Allegation.**
Evidence, statement_type=court_finding: verbatim_quote = "Marie Awad's argument that Catholic Family Services was not required to answer the appeal is disingenuous."
Allegation (ctx:allegation-044): "The court characterized Marie Awad's filings in belittling terms."
→ The court's own characterization bears on exactly what the Allegation asserts. **Create CHARACTERIZES → the Marie Awad Party AND CHARACTERIZES → ctx:allegation-044** — alongside the REBUTS → ctx:allegation-022 this same finding already carries (§5). An edge to a party, an edge to an allegation, and a polarity edge are not mutually exclusive.
...
- **If `statement_type` is `party_recitation` → this node produces NO finding-edge of any kind.** Do NOT create CORROBORATES, CONTRADICTS, REBUTS, or CHARACTERIZES from it. The court was restating a party's position, not finding it — creating a finding-edge would fabricate an adverse determination (or a characterization) the court never made. (The node still exists in the graph from Pass 1, preserved so a reviewer can see the party's position — this mirrors how discovery preserves evasive/objection answers as Evidence without CORROBORATES.)
  - A `party_recitation` STILL gets its STATED_BY (to the judge) and ABOUT edges — those are structural, not finding-edges. It is the FINDING-edges — the cross-document CORROBORATES/CONTRADICTS/REBUTS AND the local CHARACTERIZES (the court's own characterization), **to a Party or to an Allegation alike** — that are gated out. ABOUT is structural and is NOT gated, for either target: a recited position is still ABOUT its subject.
...
For each surviving finding (`court_finding` / `legal_conclusion`) whose text evaluates a party's conduct, character, competence, or motive in evaluative terms (§6), create **CHARACTERIZES → that Party** (a local Party entity in this document). This is gated like the polarity edges — a `party_recitation` never produces a CHARACTERIZES edge (Stage 1). Where that characterization also bears on what an Allegation asserts about the party, create **CHARACTERIZES → that Allegation** as well (§6) — likewise gated, so a `party_recitation` produces neither. A single finding may carry BOTH a CHARACTERIZES (to the party it labels, and to the allegation the labelling touches) and a REBUTS/CORROBORATES (to an allegation it counters/confirms) — they are not mutually exclusive.
...
2. **ABOUT:** one per party each Evidence concerns, AND one per Allegation whose subject matter it discusses (§2). Structural — therefore created for `party_recitation` nodes too.
...
```json
{
  "relationships": [
    ...
    {"relationship_type": "ABOUT", "from_entity": "evidence-031", "to_entity": "ctx:allegation-031"},
...
- [ ] For a characterization the court is merely RECITING ("CFS maintained Awad was unreasonable"), did I create NO CHARACTERIZES edge — to a Party OR to an Allegation (the PARTY is characterizing, not the court)?
- [ ] Did I still create ABOUT → Allegation for party_recitation nodes whose subject matter an Allegation concerns (ABOUT is structural, not gated)?
