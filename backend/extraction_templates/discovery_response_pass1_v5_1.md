...
-->
# Discovery Response Entity Extraction — Pass 1: Entities Only (v5.1)
...
- **Party** entities you extract must match the names already in the case graph. If you extract "George Phillips" here and the complaint has "George Phillips," the graph connects them. If you misspell the name or use a different form, the connection breaks. Use the exact name from the document.
...
- `pattern_tags`: Comma-separated tags — see taxonomy below
- `event_date`: Date referenced in the Q&A if applicable
