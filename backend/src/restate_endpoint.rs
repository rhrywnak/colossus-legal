...
pub async fn serve_restate_endpoint(listener: TcpListener) {
    // P1-7 binds the echo `DocumentPipeline` workflow — the first
    // and (for now) only service exposed by this SDK endpoint. P2
    // will add more services here as the real pipeline lands.
    let endpoint = Endpoint::builder()
        .bind(DocumentPipelineImpl.serve())
        .build();
