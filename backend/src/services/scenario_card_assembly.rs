...
fn sort_by_code(cards: &mut [ScenarioCard]) {
    cards.sort_by_key(|c| {
        let ordinal = c
            .code
            .as_deref()
            // The ONE spelling of the handle, from the module that owns it —
            // never a second literal here. See `CANDIDATE_CODE_PREFIX`.
            .and_then(|code| code.strip_prefix(crate::domain::scenario_code::CANDIDATE_CODE_PREFIX))
