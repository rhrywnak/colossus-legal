...
fn the_required_key_list_matches_what_the_snapshot_actually_reads() {
    ...
    assert_eq!(
        ...
        "seven numbers, 2.10's short-list cap, 2.11 B2's timeline threshold, \
         ...
         hide one she has to answer — and the Include picker's default stance, \
         which decides which way a candidate card's picker opens and was a \
         constant in the browser until 2026-09-19"
    );
    assert_eq!(
        WORDING_KEYS.len(),
        58,
