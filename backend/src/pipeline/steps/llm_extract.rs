...
async fn run_full_document_extraction(
    ...
    // nothing about why. Found while wiring the truncation gate (2026-08-25).
    let response = match call_with_rate_limit_retry(
        ...
        0,
        1,
        // The retry policy, read once at startup. A failed LLM call fails this
        // run and waits for a human — except a pre-generation 429/529, which
        // billed nothing and is retried on its own budget (ruled 2026-08-28).
        args.app_context.llm_retry_policy,
    )
    .await
    {
        Ok(r) => r,
        Err(e) => {
            ...
            let recorded = format!("[pass 1] {failure}");
            mark_run_failed(args.db, args.run_id, &recorded).await;
