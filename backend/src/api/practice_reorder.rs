...
pub async fn post_reorder_question(
    ...
    // ## Why this signs from the SESSION and the sibling handlers do not (yet)
    //
    // Every other deck write on this branch takes an `editing_as` string from a
    // dropdown. That picker is being removed on `feat/practice-v1-hotfix-workflow`
    // — the premise behind it (that this build has one shared login) was simply
    // wrong, and the hook refused every write while it was unset.
    //
    // A NEW endpoint has no reason to be born with the defect, so this one reads
    // the authenticated user directly. After the two branches merge it needs one
    // edit — `attribution(&user)` for the display name beside the id — and
    // nothing else. Named in the report.
    let by = user.username.clone();
    ...
    let position = commit_order(&state, &question, &order, &by).await?;
...
///
/// `by` is the signer, from the session. When this branch merges with
/// `feat/practice-v1-hotfix-workflow` it becomes the `(id, name)` pair that
/// branch's `attribution()` returns, and `NewChange` gains `changed_by_id` —
/// the compiler catches it. Named in the report.
///
