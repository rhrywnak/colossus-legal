...
pub(crate) async fn run_extract(
    ...
    let schema_file_owned = overrides
        .schema_file
    ...
    let admin_instructions_owned = overrides
        .admin_instructions
    ...
    let schema =
        colossus_extract::ExtractionSchema::from_file(Path::new(&schema_path)).map_err(|e| {
            AppError::Internal {
    ...
    let raw_template =
        std::fs::read_to_string(template_dir.join("chunk_extract.md")).map_err(|e| {
            AppError::Internal {
    ...
    .await
    .map_err(|e| AppError::Internal {
        message: format!("Failed to insert extraction run: {e}"),
    })?;
    ...
    let completeness =
        super::completeness_validation::validate_completeness(&schema, &summary.legacy_json);
    ...
    if !completeness.passed {
        ...
        return Err(AppError::BadRequest {
            message: format!(
                "Extraction completeness validation failed: {}",
...
pub async fn extract_handler(
    ...
    if document.status != "TEXT_EXTRACTED" {
        return Err(AppError::Conflict {
            message: format!(
                "Cannot extract: status is '{}', expected 'TEXT_EXTRACTED'",
                document.status
            ),
